...
%%%%%branch log%%%%%%%%%%%%%%%
f_branch =fopen('../../bin/branch_line.txt','r');
if f_branch == -1
    error('File branch_line.txt could not be opened, check name or path.')
end
line= fgetl(f_branch);
branch_log = [];
while ischar(line)
...
%%%%%%%%%%%%%%%%branch sample%%%%%%%%%%%%%%%%%%%%%%%%
f_sample_line=fopen('../../bin/sample_line.txt','r');
if f_sample_line == -1
    error('File sample_line.txt could not be opened, check name or path.')
end
line=fgetl(f_sample_line);
sample_line_log = [];
while ischar(line)
    ...
    y = log_sd{2};
    z = log_sd{3};
    sample_line_log= [sample_line_log; [x,y,z] ];
    line= fgetl(f_sample_line);
end
%%%%%%%%%%%%%%%%%%%nodes%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
