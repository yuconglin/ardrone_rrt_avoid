...
namespace user_types{
   ...
   int BirdConfig::ParamfromXML(const char* pFilename)
   {
      try
      {
        ...
        ticpp::Element *child= doc.FirstChildElement("bird_param")->FirstChildElement();
        //an iterator
   ...
   double BirdConfig::MaxAscend()
   {
      double vz_max= vz;
      return atan2(vz_max,v);
   }//MaxAscend() ends

   double BirdConfig::Ascend()
   {
      return atan2(vz,v);
   }

   void BirdConfig::NormalizeU(arma::vec::fixed<3>& u)
   {
